...
use v5.36;
our $VERSION = '0.010_001';  # Developer release - experimental optimizations
...
sub new_filter {
	...
	# Cache arrays as hashes for O(1) lookups (optimization)
	...
	my $self			= {
		...
		# Optimization: cached hash lookups
		_required_hash => { map { $_ => 1 } @$required },
		_accepted_hash => { map { $_ => 1 } @$accepted },
		_excluded_hash => { map { $_ => 1 } @$excluded },
		_has_wildcard  => grep { $_ eq '*' } @$accepted,
	};
...
sub apply {
	...
	# Optimization: pass cached hashes for faster lookups
	my @result	= filter(
		$args,
		$req,
		$ok,
...
# Fast path for hashref input (95%+ of use cases)
# This avoids type checking and array parsing overhead
sub _filter_hashref ($args_ref, $req, $ok, $no, $db, $req_hash, $acc_hash, $exc_hash, $has_wildcard) {
	my $wantarray = wantarray;

	# Fast path: assume hashref (no copy needed for hashrefs)
	my %args = $args_ref->%*;
	my @required_flds = $req->@*;

	# Quick fail checks
	my $args_count = keys %args;
	unless ($args_count) {
	...
	if ($args_count < @required_flds) {
		my $err = "Unable to initialize without all required arguments: " .
			join ', ' => map { "'$_'" } @required_flds;
		return $wantarray ? (undef, $err) : undef;
	}

	# Extract required fields using hash slice (MUCH faster than individual deletes)
	my $filtered = {};
	my @missing_required;
	my @found_required;
	...
	# Bulk copy using hash slice (optimization)
	$filtered->{$_} = $args{$_} for @found_required;
	delete @args{@found_required};

	# Fast return if nothing else to process
	if (keys(%args) == 0) {
	...
	if (@found_required == @required_flds and not $ok->@*) {
		return $wantarray ? ($filtered, "Admitted") : $filtered;
	}

	# Remove excluded fields (use hash if available, otherwise array)
	my @excluded;
	if ($exc_hash) {
		# Fast path: use cached hash for O(1) lookups
		my @to_delete = grep { exists $exc_hash->{$_} } keys %args;
		if (@to_delete) {
			@excluded = @to_delete;
			delete @args{@to_delete};
		}
	}
	else {
		for my $fld ($no->@*) {
	...
	# Accept fields
	# Use cached wildcard check if available
	my $use_wildcard = defined($has_wildcard) ? $has_wildcard : grep { $_ eq '*' } $ok->@*;

	if ($use_wildcard) {
		# Wildcard: accept all remaining fields (bulk copy)
	...
	# Debug warnings (if enabled)
	my @warnings;
	if ($db) {
		my @unrecognized = keys %args;
		if (@unrecognized) {
	...
	# Fast return path
	my $return_msg = @warnings ? join("\n" => @warnings) : "Admitted";
	return $wantarray ? ($filtered, $return_msg) : $filtered;
}

sub filter ($args,$req,$ok=[],$no=[],$db=0, $req_hash=undef, $acc_hash=undef, $exc_hash=undef, $has_wildcard=undef) {
	# Fast path for hashref with cached hashes (OO interface optimization)
	if (ref $args eq 'HASH' && defined $req_hash) {
		return _filter_hashref($args, $req, $ok, $no, $db, $req_hash, $acc_hash, $exc_hash, $has_wildcard);
	}

	# Fast path for plain hashref (functional interface)
	if (ref $args eq 'HASH') {
		return _filter_hashref($args, $req, $ok, $no, $db, undef, undef, undef, undef);
	}

	# Slow path: handle arrayref, scalar, and other formats
	my %args		= ();
	...
	my $check_wildcard = grep { $_ eq '*' } $ok->@*;
